...
# We're now using subprocess calls (proper use of system calls in Python3)
# to do this work (was using old os calls for Python2)
###############################################################################
#Let's not have any feedback from these subprocess calls (keep console clear)
with open(os.devnull, 'wb') as devnull:
	#Let's get those permissions sorted to access the terminal (in case they're not right)
	subprocess.call(['sudo', 'chmod', '666', '/dev/tty1'], stdout=devnull, stderr=devnull)
	#Turn off the blinking cursor
	subprocess.call(['setterm', '-cursor', 'off', '>', '/dev/tty1'], stdout=devnull, stderr=devnull)
	#then send a bunch of zeros to the first screen to clear it out
	subprocess.call(['sudo', 'dd', 'if=/dev/zero', 'of=/dev/fb0'], stdout=devnull, stderr=devnull)
